...
const decodeHandler = (ciphertext: string, $config: I_ConfigParam): string => {
  ...
  const keysArray: number[] = $config.prime.map(
    (prime: number) => prime * publicPrime
  );

  // 處理並轉換密文
  const resultArray: string[] = codeArray.map(
    (cipherCode: string, index: number) => {
      const cipherCodeArray: string[] = cipherCode.split('');
      let plainCode = '';

      // 將乘積列表全部帶進凱薩密碼驗證
      for (let i = 0; i < keysArray.length; i++) {
        const _plainCode: string = decodeCaesarCipher(
          cipherCodeArray,
          keysArray[i]
        );
        if (index % 10 === i) {
